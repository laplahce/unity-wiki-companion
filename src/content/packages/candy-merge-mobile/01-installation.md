...
3. Go to **Edit → Project Settings → Player → Android (or your specific Mobile Platform) → Resolution and Presentation**.
    i. Change `Auto Rotation Behaviour` to `Sensor`.
    ii. Under `Allowed Orientations for Auto Rotation`, disable both `Portrait` and `Portrait Upside Down`.
    iii. *\[Optional\]* Disable `Run Without Focus`.
4. To get the Take Screenshot functionality to work when a game is finished. Look in the script **Candy Merge → Systems → Game Over → Scripts → GameOver** in the function `TakeScreenshot()`.
> Due to permissions, other platform-specific & user-specific needs, the take screenshot functionality with vary a lot. [Check this out for a possible screenshot integration solution](https://assetstore.unity.com/packages/tools/integration/native-gallery-for-android-ios-112630).
